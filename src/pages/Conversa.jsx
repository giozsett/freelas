...
export default function Conversa() {
  ...
  useEffect(() => {
    ...
    const sincronizarConversa = async () => {
      try {
        const res = await fetch(`${API}/api/chat/${idAtual}/`, { headers: authHeaders() });
        if (!res.ok) return;
        const dados = await res.json();
        if (fechado) return;
        setMessages((prev) => {
        ...
        // ignora: o WebSocket é a fonte principal, o histórico vem do abrirChat.
      }
    };
  ...
  // Atualiza a lista de conversas quando o total global de mensagens não
  // lidas muda (evento do WebSocket de notificações) — sem polling.
  useEffect(() => {
